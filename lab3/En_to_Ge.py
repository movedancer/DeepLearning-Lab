...
class Multi30K():
    ...
    def _load(self, path):
        ...
        ret = [read_data(members['en']), read_data(members['de'])]
        return list(zip(*ret))
...
def build_vocab(dataset):
    ...
    return Vocab(en_count_dict, min_freq=2), Vocab(de_count_dict, min_freq=2)


en_vocab, de_vocab = build_vocab(train_dataset)
print('Unique tokens in de vocabulary:{} and en vocabulary:{}\n'.format(len(de_vocab), len(en_vocab)))

print("ein" in en_vocab.word2idx)  # True 应该是，说明en_vocab存储的是德文词表，de_vocab存储的是英文词表
print(en_vocab.encode(["ein", "mann", "mit"]))  # 应该返回非 <unk> 索引

str_seq_en = ['two', 'young', ',', 'white', 'males', 'are', 'outside', 'near', 'many', 'bushes', '.']
print("word:{}\nindex:{}\n".format(str_seq_en, de_vocab.encode(str_seq_en)))

index = [5, 6, 7, 8, 9, 10]
print("index:{}\nword:{}".format(index, de_vocab.decode(index)))
...
class Iterator():
    ...
    def __len__(self):
        return self.len
# 当前de_vocab存储英文，en_vocab存储德文，因此顺序不变，但是在迭代器中二者相反。
...
src_emb = nn.Embedding(len(en_vocab), 4)
...
encoder = Encoder(len(en_vocab), 512, 8, 2048, 6)

for src_idx, src_len, trg_idx in train_iterator():
    enc_outputs, enc_self_attns = encoder(src_idx, en_vocab.pad_idx)
